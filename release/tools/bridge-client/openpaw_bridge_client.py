...
class Settings:
    ...
    media_allowed_roots: tuple[Path, ...]
    once: bool
    dry_run: bool
...
def media_allowed_roots_from_env() -> tuple[Path, ...]:
    raw = os.environ.get("OPENPAW_MEDIA_ALLOWED_ROOTS", "").strip()
    if raw == "":
        return ()
    ...
    for item in raw.split(":"):
        ...
        try:
            ...
            logging.warning("Ignoring inaccessible OPENPAW_MEDIA_ALLOWED_ROOTS entry")
            continue
        if root.is_dir():
            roots.append(root)
    return tuple(roots)
...
def settings_from_env(args: argparse.Namespace) -> Settings:
    ...
    return Settings(
        ...
        media_allowed_roots=media_allowed_roots_from_env(),
...
def request_multipart_image(
    settings: Settings,
    path: str,
    image_path: Path,
) -> dict[str, Any]:
    boundary = "----openpaw-" + uuid.uuid4().hex
    filename = "upload" + image_path.suffix.lower()
    ...
    try:
        ...
        body_text = exc.read().decode("utf-8", errors="replace")
        raise BridgeError(f"HTTP {exc.code} for POST {path}: {body_text}") from exc
    except urllib.error.URLError as exc:
        raise BridgeError(f"Request failed for POST {path}: {exc.reason}") from exc

    try:
        decoded = json.loads(data.decode("utf-8"))
    except json.JSONDecodeError as exc:
        raise BridgeError(f"Invalid JSON from POST {path}") from exc
    if not isinstance(decoded, dict):
        raise BridgeError(f"Expected JSON object from POST {path}")
    return decoded
...
def validate_image_path(settings: Settings, image_path: str) -> Path:
    if settings.media_allowed_roots == ():
        raise BridgeError("OPENPAW_MEDIA_ALLOWED_ROOTS is required for image actions")
    try:
        path = Path(image_path).expanduser().resolve(strict=True)
    except OSError as exc:
        raise BridgeError("Image file is not accessible") from exc
    if not any(path_is_inside(path, root) for root in settings.media_allowed_roots):
        raise BridgeError("Image file is outside allowed roots")
    if not path.is_file():
        raise BridgeError("Image path is not a regular file")
    if path.suffix.lower() not in IMAGE_EXTENSIONS:
        raise BridgeError("Image file extension is not allowed")
    if path.stat().st_size > MAX_IMAGE_BYTES:
        raise BridgeError("Image file is larger than 10 MB")
    return path


def memory_id_from_response(payload: dict[str, Any] | None) -> str:
    if not isinstance(payload, dict):
        raise BridgeError("Memory create response did not include id")
...
def media_id_from_response(payload: dict[str, Any]) -> str:
    media = payload.get("media")
    if isinstance(media, dict):
        value = media.get("id")
        if isinstance(value, str) and value != "":
            return value
    raise BridgeError("Media upload response did not include media.id")
...
def parse_agent_stdout(stdout: str) -> AgentResult:
    text = stdout.strip()
    if text == "":
    ...
    try:
        ...
        return AgentResult(reply=text, actions=[])
    if isinstance(decoded, dict):
        for key in ("reply", "text", "content", "message"):
            ...
            if isinstance(value, str) and value.strip() != "":
                ...
                if actions is None:
                    return AgentResult(reply=value.strip(), actions=[])
                if not isinstance(actions, list):
                    raise BridgeError("Agent command actions must be a list")
                for action in actions:
                    if not isinstance(action, dict):
                        raise BridgeError("Agent command action must be an object")
                return AgentResult(reply=value.strip(), actions=actions)
    if isinstance(decoded, str) and decoded.strip() != "":
        return AgentResult(reply=decoded.strip(), actions=[])
    raise BridgeError("Agent command JSON did not contain reply text")


def run_agent(settings: Settings, claim: ClaimedMessage) -> AgentResult:
    payload = agent_payload(claim)
    if settings.dry_run:
        text = payload.get("text") or ""
        return AgentResult(reply=f"[dry-run] Paw received: {text}", actions=[])
...
def claim_source_ref(claim: ClaimedMessage) -> str:
    thread_id = claim.message.get("thread_id") or claim.message.get("chat_thread_id")
    if thread_id is None:
        thread_id = claim.message.get("conversation_id") or "unknown"
    return f"chat:{thread_id}:{claim.message_id}"


def memory_path_id(memory_id: str) -> str:
    if re.fullmatch(r"[A-Za-z0-9._:-]+", memory_id) is None:
        raise BridgeError("Memory id contains unsupported characters")
    return memory_id


def create_action_memory(settings: Settings, action: dict[str, Any]) -> str:
    existing_memory_id = action.get("memory_id")
    if isinstance(existing_memory_id, str) and existing_memory_id.strip() != "":
        return existing_memory_id.strip()

    memory = action.get("memory")
    if not isinstance(memory, dict):
        raise BridgeError("store_image_memory action requires memory or memory_id")
    payload = dict(memory)
    response = request_json(settings, "POST", "/memories", payload)
    return memory_id_from_response(response)


def create_attachment_payload(
    claim: ClaimedMessage,
    action: dict[str, Any],
    media_id: str,
    image_path: Path,
) -> dict[str, Any]:
    raw_attachment = action.get("attachment")
    attachment = dict(raw_attachment) if isinstance(raw_attachment, dict) else {}
    attachment["media_id"] = media_id
    attachment.setdefault("role", "image")
    attachment.setdefault("source", "openclaw")
    attachment.setdefault("source_ref", claim_source_ref(claim))
    attachment.setdefault("original_filename", image_path.name)
    return attachment


def execute_store_image_memory(
    settings: Settings,
    claim: ClaimedMessage,
    action: dict[str, Any],
) -> None:
    ...
    if not isinstance(raw_path, str) or raw_path.strip() == "":
        raise BridgeError("store_image_memory action requires image_path")

    image_path = validate_image_path(settings, raw_path)
    memory_id = create_action_memory(settings, action)
    media_response = request_multipart_image(settings, "/media", image_path)
    media_id = media_id_from_response(media_response)
    attachment_payload = create_attachment_payload(claim, action, media_id, image_path)
    request_json(
        settings,
        "POST",
        f"/memories/{memory_path_id(memory_id)}/attachments",
        attachment_payload,
    )
    logging.info(
        "Stored image memory attachment for message %s memory %s media %s",
        claim.message_id,
        memory_id,
        media_id,
    )
...
def execute_actions(
    ...
    for action in actions:
        ...
        if action_type == "store_image_memory":
            execute_store_image_memory(settings, claim, action)
            continue
        raise BridgeError(f"Unsupported agent action type: {action_type}")
...
def process_once(settings: Settings) -> bool:
    ...
    agent_result = run_agent(settings, claim)
    execute_actions(settings, claim, agent_result.actions)
    post_reply(settings, claim, agent_result.reply)
