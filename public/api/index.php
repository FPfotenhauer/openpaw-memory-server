...
function create_memory(PDO $pdo, array $payload): array
{
    ...
    $kind = clean_limited_string($payload['kind'] ?? 'note', 'kind', 64);
    $importance = parse_unit_float($payload['importance'] ?? 0.5, 'importance');
    $scope = clean_limited_string($payload['scope'] ?? 'personal', 'scope', 64);
...
function update_memory(PDO $pdo, string $id, array $payload): ?array
{
    ...
    $kind = array_key_exists('kind', $payload) ? clean_limited_string($payload['kind'], 'kind', 64) : $current['kind'];
    $importance = array_key_exists('importance', $payload)
        ? parse_unit_float($payload['importance'], 'importance')
        : $current['importance'];
    $scope = array_key_exists('scope', $payload) ? clean_limited_string($payload['scope'], 'scope', 64) : $current['scope'];
